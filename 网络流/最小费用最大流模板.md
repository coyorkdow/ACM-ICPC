...
public:
    ...
    mcmf(int n, vector<int> *_g, int *_ptr, _TC *_dis, _TC *_h)
        : maxn(n), maxm(0), size(0),
          G(_g), ptr(_ptr), dis(_dis), h(_h), negative(false), always_bellman(false), eps(1e-6)
    {
    ...
    inline int add_edge(const int &u, const int &v, const _EW &cap, const _EC &cost)
    {
        ...
        {
            E[size - 2] = edge{v, cap, cost};
            E[size - 1] = edge{u, 0, -cost};
        }
        return size - 2;
    }
    inline void set_inf(const _TC &_inf, const _EW &_minf)
    {
        inf = _inf;
        minf = _minf;
    }
